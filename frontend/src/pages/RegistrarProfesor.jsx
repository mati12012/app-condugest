...
const RegistrarProfesor = ({ cambiarVista }) => {
    ...
    const validarRutChileno = (rut) => {
        ...
        return /^(\d{1,2}\.?\d{3}\.?\d{3}-[\dkK])$/.test(rutLimpio);
    };

    const validarTextoPersona = (texto) => {
    ...
    const validarFormulario = () => {
        ...
        if (!validarTextoPersona(datos.nombre)) {
        ...
        if (!validarTextoPersona(datos.apellido)) {
            return 'Error: El apellido solo debe contener letras';
        }
        ...
        if (datos.licencia_autorizada.trim() === '') {
            return 'Error: La licencia autorizada es obligatoria';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setMensaje(
                    `Error: ${respuestaServidor.message || 'No se pudo registrar el profesor'}`
                );
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="w-full max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-slate-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {mensaje && (
                    <div className={`p-4 rounded-lg font-medium text-sm ${mensaje.toLowerCase().includes('error')
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                        Puede ingresarse con o sin puntos. Ejemplo: 12345678-9.
                    </p>
                </div>
                ...
                <div>
                    ...
                    />
                    <p className="text-xs text-slate-400 mt-1">
                        Campo opcional. Ejemplo recomendado: +56912345678.
                    </p>
                </div>
