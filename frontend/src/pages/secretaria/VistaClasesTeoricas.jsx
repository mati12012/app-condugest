...
const VistaClasesTeoricas = ({ cambiarVista }) => {
  ...
  const getColorModalidad = (modalidad) => {
    switch (modalidad) {
      case 'Presencial': return 'bg-slate-100 text-slate-700 border-slate-200';
      case 'Online': return 'bg-indigo-100 text-indigo-700 border-indigo-200';
      case 'Híbrida': return 'bg-amber-100 text-amber-700 border-amber-200';
      default: return 'bg-slate-100 text-slate-600 border-slate-200';
    }
  };

  const obtenerTextoSala = (clase) => {
    if (clase.salaTeorica?.nombre) {
      return `${clase.salaTeorica.nombre} - ${clase.salaTeorica.sede}`;
    }
  ...
  const clasesFiltradas = clases.filter((clase) => {
    ...
    const modalidad = (clase.modalidad || '').toLowerCase();
    const sala = (clase.salaTeorica?.nombre || '').toLowerCase();
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-200 bg-slate-50 flex flex-col gap-3">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <input
              type="text"
              placeholder="Buscar por tema, profesor, modalidad, sala, sede o fecha..."
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-100 text-slate-600 text-sm">
              <tr>
                ...
                <th className="p-4 font-bold">Modalidad y sala</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {clasesFiltradas.map((clase) => (
                <tr key={clase.id_clase_teorica} className="hover:bg-slate-50">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex text-xs font-bold px-2 py-1 rounded-full border ${getColorModalidad(clase.modalidad)}`}>
                      {clase.modalidad || 'Sin modalidad'}
                    </span>
                    <div className="text-xs text-slate-500 mt-2 font-medium">{obtenerTextoSala(clase)}</div>
                    {clase.link_reunion && (
                      <div className="text-xs text-indigo-600 mt-1 font-semibold">Link registrado</div>
                    )}
                  </td>
                    <td className="p-4">
                      <select
