...
const EditarClaseTeorica = ({ idClase, cambiarVista }) => {
  ...
  const cargarDatosIniciales = useCallback(async () => {
    try {
      setCargando(true);
      const [resProfesores, resSalas, resClase] = await Promise.all([
      ...
      if (resSalas.ok) setSalas(dataSalas.data || []);

      if (resClase.ok) {
        const c = dataClase.data;
        const modalidad = c.modalidad || (c.sede === "Online" ? "Online" : "Presencial");

        setDatos({
          tema: c.tema || "",
          fecha: String(c.fecha || "").split("T")[0],
          hora_inicio: String(c.hora_inicio || "").slice(0, 5),
          hora_fin: String(c.hora_fin || "").slice(0, 5),
          sede: modalidad === "Online" ? "Online" : c.salaTeorica?.sede || c.sede || "",
          estado: c.estado || "Programada",
          modalidad,
          id_sala_teorica: c.salaTeorica?.id_sala_teorica || "",
          link_reunion: c.link_reunion || "",
          codigo_reunion: c.codigo_reunion || "",
          url_grabacion: c.url_grabacion || "",
          id_profesor: c.profesor ? c.profesor.id_profesor : "",
        });
      } else {
        setErroresCampos(obtenerMensajeError(dataClase, "No se pudo cargar la clase."));
      }
    } catch (error) {
      console.error(error);
  ...
  const idSalaSeleccionada = datos?.id_sala_teorica;

  const salaSeleccionada = useMemo(
    () => salas.find((sala) => Number(sala.id_sala_teorica) === Number(idSalaSeleccionada)),
    [idSalaSeleccionada, salas]
  );

  const salasDisponibles = useMemo(() => {
    return salas.filter((sala) => (
      sala.estado === "Activa"
      || Number(sala.id_sala_teorica) === Number(idSalaSeleccionada)
    ));
  }, [idSalaSeleccionada, salas]);
  ...
  const profesoresDisponibles = useMemo(() => {
    ...
    return profesores
      .filter((profesor) => profesor.estado === true || Number(profesor.id_profesor) === Number(datos.id_profesor))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setMensajeExito("Clase teorica actualizada exitosamente.");
        setTimeout(() => cambiarVista("clasesTeoricas"), 1200);
      } else {
        setErroresCampos(obtenerMensajeError(respuestaServidor, "Error al actualizar la clase."));
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-slate-200">
      <div className="mb-6 border-b pb-4 flex justify-between items-start gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Editar Clase Teorica</h2>
          <p className="text-slate-500 mt-1">Modifique los detalles de la sesion.</p>
        </div>
        <button
          type="button"
              ...
              >
                ...
                {salasDisponibles.map((sala) => (
                  <option key={sala.id_sala_teorica} value={sala.id_sala_teorica}>
                    {sala.nombre} - {sala.sede} ({sala.capacidad} personas){sala.estado !== "Activa" ? " - Inactiva" : ""}
                  </option>
                ))}
              </select>
            </div>
